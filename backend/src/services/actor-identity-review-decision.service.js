...
function approvalBlockers(run, adapter) {
  ...
  if (futureValidation.independentNeutralMotionSourceUsed !== true) blockers.push('INDEPENDENT_NEUTRAL_MOTION_NOT_PROVEN')
  if (text(futureValidation.controlRepresentation) !== 'softedge_ffmpeg_edgedetect_v1') blockers.push('SOFTEDGE_CONTROL_NOT_PROVEN')
  if (futureValidation.appearanceReducedStructuralControlUsed !== true) blockers.push('APPEARANCE_REDUCED_CONTROL_NOT_PROVEN')
  if (futureValidation.rawRgbControlUsed !== false) blockers.push('RAW_RGB_CONTROL_FORBIDDEN')
  if (futureValidation.sameControlAcrossBranches !== true) blockers.push('CONTROL_NOT_PAIRED_ACROSS_ABC')
  if (futureValidation.sameSeedAcrossBranches !== true) blockers.push('SEED_NOT_PAIRED_ACROSS_ABC')
  if (futureValidation.sameSamplerAcrossBranches !== true) blockers.push('SAMPLER_NOT_PAIRED_ACROSS_ABC')
  if (futureValidation.baselineWithoutIdentityAvailable !== true) blockers.push('BASELINE_WITHOUT_IDENTITY_MISSING')
  if (futureValidation.identityReferenceWithoutLoraAvailable !== true) blockers.push('IDENTITY_REFERENCE_WITHOUT_LORA_MISSING')
  if (futureValidation.candidateWithLoraAvailable !== true) blockers.push('LORA_CANDIDATE_MISSING')
  if (futureValidation.loraIsolationComparisonAvailable !== true) blockers.push('LORA_ISOLATION_COMPARISON_MISSING')
